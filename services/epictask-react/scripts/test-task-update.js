...
async function testTaskUpdatedEndpoint() {
  try {
    ...
    console.log("❌ TaskUpdated endpoint test failed:");
    if (error.response) {
...
function testTaskServiceStructure() {
  ...
  const hasTaskId =
    expectedStructure.task_id && typeof expectedStructure.task_id === "string";
  const hasUpdatedFields =
    expectedStructure.updated_fields &&
    typeof expectedStructure.updated_fields === "object";
...
async function runTests() {
  ...
  console.log("\n=== Endpoint Test ===");
  console.log(
    "Note: This test requires the task management service to be running on localhost:8081",
  );
  console.log(
    "If the service is not running, this test will fail but the integration should still work when the service is available.\n",
  );
  ...
  console.log(
    `Endpoint Test: ${endpointTest ? "✅ PASS" : "❌ FAIL (service may not be running)"}`,
  );
  ...
  if (structureTest) {
    ...
    console.log(
      "The handleTaskSave function will now properly call the TaskUpdated endpoint.",
    );
  } else {
